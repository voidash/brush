// Package logo renders a Brush wordmark in a stylized way.
package logo
...
// Opts are the options for rendering the Brush title art.
type Opts struct {
	FieldColor   color.Color // diagonal lines
	TitleColorA  color.Color // left gradient ramp point
	TitleColorB  color.Color // right gradient ramp point
	CharmColor   color.Color // funky text color
	VersionColor color.Color // Version text color
	Width        int         // width of the rendered logo, used for truncation
}

// Render renders the Brush logo. Set the argument to true to render the narrow
...
func Render(version string, compact bool, o Opts) string {
	...
	brush := renderWord(spacing, stretchIndex, letterforms...)
	brushWidth := lipgloss.Width(brush)
	b := new(strings.Builder)
	for r := range strings.SplitSeq(brush, "\n") {
	...
	// Charm and version.
	metaRowGap := 1
	maxVersionWidth := brushWidth - lipgloss.Width(charm) - metaRowGap
	version = ansi.Truncate(version, maxVersionWidth, "…") // truncate version if too long.
	gap := max(0, brushWidth-lipgloss.Width(charm)-lipgloss.Width(version))
	metaRow := fg(o.CharmColor, charm) + strings.Repeat(" ", gap) + fg(o.VersionColor, version)

	// Join the meta row and big Brush title.
	brush = strings.TrimSpace(metaRow + "\n" + brush)

	// Narrow version.
	if compact {
		field := fg(o.FieldColor, strings.Repeat(diag, brushWidth))
		return strings.Join([]string{field, field, brush, field, ""}, "\n")
	}

	fieldHeight := lipgloss.Height(brush)
	...
	// Right field.
	rightWidth := max(15, o.Width-brushWidth-leftWidth-2) // 2 for the gap.
	const stepDownAt = 0
	...
	const hGap = " "
	logo := lipgloss.JoinHorizontal(lipgloss.Top, leftField.String(), hGap, brush, hGap, rightField.String())
	if o.Width > 0 {
...
// SmallRender renders a smaller version of the Brush logo, suitable for
// smaller windows or sidebar usage.
func SmallRender(width int) string {
	t := styles.CurrentTheme()
	title := t.S().Base.Foreground(t.Secondary).Render("funky")
	title = fmt.Sprintf("%s %s", title, styles.ApplyBoldForegroundGrad("Brush", t.Secondary, t.Primary))
	remainingWidth := width - lipgloss.Width(title) - 1 // 1 for the space after "Brush"
	if remainingWidth > 0 {
...
// letterB renders letter B in a stylized way. It takes an integer that
// determines how many cells to stretch the letter. If stretch is less than
